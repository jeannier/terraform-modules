...
# container insight setup
#
# inspired from https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Container-Insights-setup-EKS-quickstart.html
# conversion done with https://github.com/sl1pm4t/k2tf (with --tf12format)
#
# automount_service_account_token was added to the two daemonsets
#
# built dynamically :
# - kubernetes_config_map cluster_info
# - kubernetes_config_map cwagentconfig
#
# added empty_dir {} to volume "fluentdconf"
  ...
  spec {
    ...
    template {
      ...
      spec {
        ...
        # IMPORTANT - it's not set in https://github.com/aws-samples/amazon-cloudwatch-container-insights/blob/master/k8s-deployment-manifest-templates/deployment-mode/daemonset/container-insights-monitoring/quickstart/cwagent-fluentd-quickstart.yaml
...
resource "kubernetes_daemonset" "fluentd_cloudwatch" {
  ...
  spec {
    selector {
      match_labels = { k8s-app = "fluentd-cloudwatch" }
    }
    template {
      metadata {
        labels      = { k8s-app = "fluentd-cloudwatch" }
        annotations = { configHash = "8915de4cf9c3551a8dc74c0137a3e83569d28c71044b0359c2578d2e0461825" }
      }
      spec {
